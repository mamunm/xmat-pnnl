...
#lgb.parity_plot(data='train', quantity='CT_RT').savefig('parity_CT_RT_train.png')
#lgb.parity_plot(data='test', quantity='CT_RT').savefig('parity_CT_RT_test.png')
np.save('lgb_dict.npy', lgb.__dict__)
plot_importance(lgb.model[0])
plt.show()
'''
plt.clf()
plot_metric(lgb.model[0], metric='rmse')
plt.show()
plt.clf()
plot_tree(lgb.model[0])
plt.show()
'''
