...
test_pcc = np.array([data[i]['pr_mean_test'] for i in data.keys()][::-1])
test_pcc_std = np.array([data[i]['pr_std_test'] for i in data.keys()][::-1])
train_pcc = np.array([data[i]['pr_mean_train'] for i in data.keys()][::-1])
train_pcc_std = np.array([data[i]['pr_std_train'] for i in data.keys()][::-1])
plt.figure(figsize=(10, 8))
plt.title('Learning Curve [Catboost Algorithm]',
        fontsize=18)
