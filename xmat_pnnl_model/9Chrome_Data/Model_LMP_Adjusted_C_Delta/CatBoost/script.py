...
from xmat_pnnl_code import GBM_Delta
...
features = [i for i in df.columns if i not in ['CT_RT', 'CT_Temp', 
    'ID', 'CT_CS', 'LMP_Model', 'CT_MCR', 'log_CT_CS']]
X = df[features].to_numpy(np.float32)
y = df['LMP_Model'].to_numpy(np.float32)
X_lin = df[['log_CT_CS']].to_numpy(np.float32)
y2 = df[['ID', 'CT_RT', 'CT_Temp', 'CT_CS', 'log_CT_CS']].values.tolist()
...
X_lin = np.array([i[4] for i in data['y2']]).reshape(-1, 1)

catboost = GBM_Delta(package='catboost',
          X_lin = X_lin,
          X=data['X'],
          y=data['y'],
          model_scheme='LMP',
