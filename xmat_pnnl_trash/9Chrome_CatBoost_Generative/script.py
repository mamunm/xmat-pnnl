...
#X_for_gen = np.concatenate([data['X'], data['y'].reshape(-1, 1)], axis=1)
ae = AutoEncoder(arch=[33, 25, 20, 16, 11], 
        X=data['X'], loss='mse', epochs=250)

ae.build_model()
#plot_model(ae.vae, to_file='model.png')
#plot_model(ae.decoder, to_file='decoder.png')
X_gen = ae.get_random_alloy(200)
y_gen = scale.inverse_transform(X_gen)[:, -1]
X_gen = X_gen[:, :-1]
X_orig = data['X'][:, :-1]
y_orig = data['y']
