...
ae = AutoEncoder(arch=[data['X'].shape[1], 20, 16],
        X=data['X'], loss='mse', epochs=500)

ae.build_model()
X_gen = ae.get_linspace_alloy(100)
X_gen = scale.inverse_transform(X_gen)
y_gen = X_gen[:, -1]
X_gen = X_gen[:, :-1]
y_real = scale.inverse_transform(data['X'])[:, -1]
X_real = scale.inverse_transform(data['X'])[:, :-1]

print(X_gen)
print(X_real)

X_pred = ae.vae.predict(data['X'])
print(scale.inverse_transform(X_pred))
np.save('val_data_chrome_1.npy', {'X_gen': X_gen, 'y_gen': y_gen, 
